...
SELECT infera_get_loaded_models() as models_initial;

-- Test 2: List models (should be empty initially)
SELECT infera_get_loaded_models() as empty_models;
...
-- SELECT infera_load_model('iris_classifier', '/path/to/iris_model.onnx') as load_success;

-- Test 4: List loaded models
-- SELECT infera_get_loaded_models() as loaded_models;

-- Test 5: Get model information
-- SELECT infera_get_model_info('iris_classifier') as model_details;
...
--     petal_length,
--     petal_width,
--     infera_predict('iris_classifier', sepal_length, sepal_width, petal_length, petal_width) as prediction
...
-- SELECT
--     id,
--     infera_predict_multi('iris_classifier', sepal_length, sepal_width, petal_length, petal_width) as predictions_json
-- FROM sample_features
-- LIMIT 5;
...
--     COUNT(*) as count,
--     AVG(infera_predict('iris_classifier', sepal_length, sepal_width, petal_length, petal_width)) as avg_prediction,
--     MIN(infera_predict('iris_classifier', sepal_length, sepal_width, petal_length, petal_width)) as min_prediction,
--     MAX(infera_predict('iris_classifier', sepal_length, sepal_width, petal_length, petal_width)) as max_prediction
-- FROM sample_features
...
-- .timer on
-- SELECT COUNT(*), AVG(infera_predict('iris_classifier', feature1, feature2, feature3, feature4)) as avg_pred
-- FROM large_batch;
-- .timer off
...
-- Test 10: Extension availability check via infera_get_loaded_models()
SELECT
    CASE
        WHEN infera_get_loaded_models() IS NOT NULL THEN 'Extension loaded successfully'
...
--         ELSE CAST(infera_predict('iris_classifier', f1, f2, f3, f4) as VARCHAR)
--     END as result
-- FROM test_nulls;
...
-- SELECT infera_unload_model('iris_classifier') as unload_success;

-- Test 13: Verify model is unloaded
-- SELECT infera_get_loaded_models() as models_after_unload;
