...
static void HelloWorldInfera(DataChunk &args, ExpressionState &state,
                             Vector &result) {
  // Call the Rust function (allocates a C string)
  char *response_from_rust = hello_infera();

  // Copy into DuckDB Value and then free the original allocation to avoid
  // leaking For a zero-arg scalar function DuckDB will produce a single row in
  // the result chunk
...
static void LoadInternal(ExtensionLoader &loader) {
  // Register the function with the new ExtensionLoader API (ExtensionUtil
  // removed upstream)
  ScalarFunction hello_func("hello_infera", {}, LogicalType::VARCHAR,
                            HelloWorldInfera);
...
DUCKDB_CPP_EXTENSION_ENTRY(infera, loader) { duckdb::LoadInternal(loader); }
