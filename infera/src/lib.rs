...
struct OnnxModel { name: String, }

static MODELS: Lazy<RwLock<HashMap<String, OnnxModel>>> =
...
pub extern "C" fn infera_load_onnx_model(name: *const c_char, path: *const c_char) -> i32 {
    let result = (|| -> Result<(), InferaError> {
        ...
        let path_or_url_str = unsafe { CStr::from_ptr(path) }.to_str().map_err(|_| InferaError::Utf8Error)?;

        let local_path = if path_or_url_str.starts_with("http://") || path_or_url_str.starts_with("https://") {
...
fn load_model_impl(name: &str, path: &str) -> Result<(), InferaError> {
    let model = tract_onnx::onnx()
        ...
        .into_runnable()
        .map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let input_facts = model.model().input_fact(0).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let output_facts = model.model().output_fact(0).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let input_shape: Vec<i64> = input_facts.shape.iter().map(|d| d.to_i64().unwrap_or(-1)).collect();
    let output_shape: Vec<i64> = output_facts.shape.iter().map(|d| d.to_i64().unwrap_or(-1)).collect();
    let onnx_model = OnnxModel { model, input_shape, output_shape, name: name.to_string() };
...
pub extern "C" fn infera_unload_onnx_model(name: *const c_char) -> i32 {
    let result = (|| -> Result<(), InferaError> {
        ...
        let name_str = unsafe { CStr::from_ptr(name) }.to_str().map_err(|_| InferaError::Utf8Error)?;
        if MODELS.write().remove(name_str).is_some() { Ok(()) } else { Err(InferaError::ModelNotFound(name_str.to_string())) }
    })();
    match result {
        Ok(()) => 0,
        Err(e) => { set_last_error(&e); -1 }
...
pub extern "C" fn infera_run_inference(model_name: *const c_char, data: *const f32, rows: usize, cols: usize) -> InferaInferenceResult {
    ...
    match result {
        ...
        Err(e) => {
            set_last_error(&e);
            InferaInferenceResult { data: std::ptr::null_mut(), len: 0, rows: 0, cols: 0, status: -1, }
        }
    }
}

#[cfg(feature = "tract")]
fn run_inference_impl(model_name: &str, data: *const f32, rows: usize, cols: usize) -> Result<InferaInferenceResult, InferaError> {
    ...
    let input_tensor = Tensor::from_shape(&[rows, cols], input_data).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let outputs = model.model.run(tvec!(input_tensor.into())).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let output_tensor = outputs.into_iter().next().ok_or_else(|| InferaError::OnnxError("No output tensor".to_string()))?;
    let output_array = output_tensor.to_array_view::<f32>().map_err(|e| InferaError::OnnxError(e.to_string()))?;
...
#[cfg(not(feature = "tract"))]
fn run_inference_impl(_model_name: &str, _data: *const f32, _rows: usize, _cols: usize) -> Result<InferaInferenceResult, InferaError> {
    Err(InferaError::FeatureNotEnabled("ONNX inference requires 'tract' feature to be enabled".to_string()))
}
...
#[cfg(feature = "tract")]
fn run_inference_blob_impl(model_name: &str, blob_data: *const u8, blob_len: usize) -> Result<InferaInferenceResult, InferaError> {
    let models = MODELS.read();
    let model = models.get(model_name).ok_or_else(|| InferaError::ModelNotFound(model_name.to_string()))?;
    if blob_len % mem::size_of::<f32>() != 0 { return Err(InferaError::InvalidBlobSize); }
    let blob_bytes = unsafe { std::slice::from_raw_parts(blob_data, blob_len) };
    let float_vec: Vec<f32> = blob_bytes.chunks_exact(4).map(|chunk| f32::from_ne_bytes(chunk.try_into().unwrap())).collect();
    let expected_elements: usize = model.input_shape.iter().filter(|&&d| d > 0).map(|&d| d as usize).product();
    if float_vec.len() % expected_elements != 0 {
        return Err(InferaError::BlobShapeMismatch { expected: expected_elements, actual: float_vec.len(), });
    }
    ...
    let input_tensor = Tensor::from_shape(&final_shape, &float_vec).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let outputs = model.model.run(tvec!(input_tensor.into())).map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let output_tensor = outputs.into_iter().next().ok_or_else(|| InferaError::OnnxError("No output tensor".to_string()))?;
    let output_array = output_tensor.to_array_view::<f32>().map_err(|e| InferaError::OnnxError(e.to_string()))?;
    let output_shape = output_array.shape();
...
#[cfg(not(feature = "tract"))]
fn run_inference_blob_impl(_model_name: &str, _blob_data: *const u8, _blob_len: usize) -> Result<InferaInferenceResult, InferaError> {
    Err(InferaError::FeatureNotEnabled("ONNX inference requires 'tract' feature to be enabled".to_string()))
}
                    ...
                    if let Some(name) = file_path.file_stem().and_then(|s| s.to_str()) {
                        if let Some(full_path) = file_path.to_str() {
                            match load_model_impl(name, full_path) {
                                ...
                                Err(e) => errors.push(json!({ "file": full_path, "error": e.to_string() })),
                            }
                        }
                    }
                }
